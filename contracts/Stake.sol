...
contract TokenStake is ReentrancyGuard, Ownable {
  ...
  function migrate(address previous, address[] memory people, uint256[] memory lastClaims) external {
    ...
    for (uint i = 0; i < people.length; i++) {
      uint256 staked = TokenStake(previous).staked(people[i]);
    ...
    require(_Token.transferFrom(previous, address(this), _Token.balanceOf(previous)));
    _migrated = true;
  }
  ...
  function decreaseStake(uint256 amount) external {
    ...
    require(_Token.transfer(address(msg.sender), amount));
  ...
  function _calculateMintage(address staker) private view returns (uint256) {
    // total supply
    uint256 share = _Token.totalSupply()
