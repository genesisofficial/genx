...
contract DAO is ReentrancyGuard {
  ...
  // Proposal fee of 10 TOKEN. Used to prevent spam
  ...
  function _createNewProposal(ProposalType pType) internal active returns (uint64) {
    ...
    require(_TOKEN.transferFrom(msg.sender, address(this), PROPOSAL_FEE));
  ...
  function proposeStakeUpgrade(address newStake, address[] calldata owned, string calldata info) external returns (uint64) {
    ...
    // Ensure the TOKEN token was included as an owned contract
    for (uint i = 0; i < owned.length; i++) {
      if (owned[i] == address(_TOKEN)) {
        break;
      }
  ...
  // Send the TOKEN held by this contract to what it upgraded to
  ...
  function forwardVYBE() public {
    require(_upgrade != address(0));
    require(_TOKEN.transfer(_upgrade, _TOKEN.balanceOf(address(this))));
  }
  ...
  function completeProposal(uint64 proposalID, address[] calldata stakers) external active pendingProposal(proposalID) noReentrancy {
    ...
    if (meta.pType == ProposalType.Fund) {
      ...
      require(_TOKEN.transfer(proposal.destination, proposal.amount));
      ...
      _stake = IStake(proposal.newStake);
